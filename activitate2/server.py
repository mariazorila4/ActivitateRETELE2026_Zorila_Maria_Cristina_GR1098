    ...
    try:
        ...
        if comanda == 'CONNECT':
            ...
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Trebuie sa fii conectat pentru a publica."
            elif argumente=='':
            ...
            if adresa_client not in clienti_conectati:
                raspuns="EROARE: Trebuie sa fii conectat pentru a sterge un mesaj."
            elif not argumente.isdigit():
                raspuns="EROARE: Argumentul trebuie sa fie un ID."
            else:
                id_sters=int(argumente)
                if id_sters not in mesaje:
                    raspuns=f"EROARE: Nu exista niciun mesaj cu ID={id_sters}."
                elif mesaje[id_sters]['autor']!=adresa_client:
                    raspuns="EROARE: Nu poti sterge acest mesaj deoarece nu-ti apartine."
                else:
                    del mesaje[id_sters]
                    raspuns=f"OK: Mesajul cu ID={id_sters} a fost sters cu succes."

        elif comanda == 'LIST':
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Comanda LIST nu este inca implementata."
            elif len(mesaje)==0:
                raspuns="OK: Momentan nu exista niciun mesaj publicat pe server."
            else:
                raspuns="OK: Lista mesajelor:\n"
                for msg_id, detalii in mesaje.items():
                    raspuns+=f"[ID={msg_id}] {detalii['text']}\n"
