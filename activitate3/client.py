...
class FTPClient:
    ...
    def rename_file(self):
        ...
        res = self.send_command({'command': 'rename_file', 'old_name': old_name, 'new_name': new_name})
        print(f"{'✓' if res['status'] == 'success' else '✗'} {res['message']}")
    ...
    def read_file(self):
        ...
        print("\n📖 READ FILE (Server)")
        self.list_files()
        filename = input("Select file: ").strip()
        res = self.send_command({'command': 'read_file', 'filename': filename})
        if res['status'] == 'success':
            print(f"--- Content ---\n{res['content']}\n----------------")
        else: print(f"✗ {res['message']}")
    
    def download(self):
        """STUDENT TASK: Download file from server to local"""
        print("\n📥 DOWNLOAD FILE")
        self.list_files()
        filename = input("Downloaded file: ").strip()
        res = self.send_command({'command': 'download', 'filename': filename})
        if res['status'] == 'success':
            with open(os.path.join(LOCAL_FILES_DIR, filename), 'w') as f:
                f.write(res['content'])
            print(f"✓ Saved in {LOCAL_FILES_DIR}/")
        else: print(f"✗ {res['message']}")
    ...
    def edit_file(self):
        ...
        print("\n🛠️  EDIT FILE (Server)")
        self.list_files()
        filename = input("Edited file: ").strip()
        content = input("New content: ")
        res = self.send_command({'command': 'edit_file', 'filename': filename, 'content': content})
        print(f"✓ {res['message']}")
    
    def see_file_operation_history(self):
        """STUDENT TASK: See file operation history on server"""
        print("\n📜 SEE FILE OPERATION HISTORY")
        filename = input("Nume fisier: ").strip()
        res = self.send_command({'command': 'see_file_operation_history', 'filename': filename})
        if res['status'] == 'success':
            print(f"--- Log ---\n{res['history']}-----------")
        else: print(f"✗ {res['message']}")
